...
figures/fig_schematic_pes3.png
"""
import os
...
# --------------------------------- die NEB-Historie: alle gespeicherten Bilder
# Transition1x speichert jede Iteration des Bandes, nicht nur das Ergebnis;
# das Modell wird auf allen trainiert. Skizze: fruehe Iterationen streuen weit
# um den Pfad, spaete liegen dicht darauf; die Streuung ist an der Barriere am
# groessten, wo ein Band am laengsten wandert. Innere Bilder nur (Endpunkte
# bleiben fest).
...
for it in range(n_iter):
    f = 1.0 - it / n_iter
    for x0 in bx[1:-1]:
        sy = (0.13 * f ** 1.3 + 0.010) * (0.45 + np.exp(-((x0 - XT) / 0.24) ** 2))
        sx_ = 0.06 * f + 0.006
        cx.append(x0 + rng.normal(0.0, sx_))
        cy.append(rng.normal(0.0, sy))
...
def pes_map(ax, Z, title):
    ...
    # die NEB-Historie als Punktwolke, unter dem Band
    ax.scatter(cx, cy, s=8, c='white', alpha=0.6, lw=0, zorder=3)
...
aT.text(0.02, 0.97, 'white dots: every geometry stored during the\n'
        'Transition1x NEB optimisation, all iterations —\n'
        'the model is trained on all of them',
        transform=aT.transAxes, fontsize=7.5, color='white', ha='left',
        va='top', bbox=dict(boxstyle='round,pad=0.3', fc='black', ec='none',
                            alpha=0.7))
...
sT.scatter(cx, E_T(cx, cy), s=5, c='#9a9a9a', alpha=0.35, lw=0, zorder=2,
           label='NEB history: every stored iteration, on or above the path')
...
aB.text(0.02, 0.97, 'same NEB history as above: it clusters around\n'
        'the restricted path, not around the UKS saddle',
        transform=aB.transAxes, fontsize=7.5, color='white', ha='left',
        va='top', bbox=dict(boxstyle='round,pad=0.3', fc='black', ec='none',
                            alpha=0.7))
...
fig.text(0.04, 0.935, 'top: the level the band was relaxed at.   '
         ...
         'white dots: the whole NEB history the model was trained on.',
