"""
NEB using UMA (Universal Model for Atoms, facebook/UMA) via fairchem ASE calculator.

Drop-in analogue of orca_neb.py / esen_neb.py — identical NEB logic, only the
calculator changes. task_name='omol' routes UMA to its molecule-optimised head.
Outputs are compatible with the existing analysis pipeline.

Pipeline:
  1. Load final wB97x NEB images from Transition1x.h5 as starting band
  2. Relax endpoints with UMA / BFGS
  3. Run NEB → CI-NEB using NEBOptimizer
  4. Save neb.db + fmaxs.json (same structure as orca_neb.py)

Results stored in ~/uma_neb_results/{reaction}/

Usage:
    python uma_neb.py \
        --h5file     ~/data/Transition1x.h5 \
        --reaction   rxn7949 \
        --output     ~/uma_neb_results/rxn7949 \
        --checkpoint ~/checkpoints/uma-s-1p2.pt
"""
import argparse
import json
import os
...
def make_uma_calc(checkpoint_path):
...
def plot_mep(images, output):
    ...
    ax.plot(fit.fit_path, fit.fit_energies, 'b-o', markersize=4,
            label=f'UMA barrier: {max(fit.fit_energies):.3f} eV')
...
def assign_calcs(images, checkpoint_path):
    """Assign UMA calculator to all images.
    ...
    task_name='omol' routes UMA to its molecule-optimised head.
    """
    from fairchem.core import pretrained_mlip, FAIRChemCalculator
    predict_unit = pretrained_mlip.load_predict_unit(checkpoint_path, device='cuda')
    for atoms in images:
...
def main(args):
    ...
    if not os.path.exists(args.checkpoint):
        print(f'ERROR: UMA checkpoint not found: {args.checkpoint}')
        sys.exit(1)
    ...
    print(f'Attaching UMA calculator (checkpoint: {args.checkpoint}) ...')
    ...
    print(f'  neb.db            — UMA energies + forces')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint',  required=True, help='Path to UMA .pt checkpoint')
