"""
Train the FIXED delta correction head on top of frozen MACE.

Fix relative to train_delta_head.py (the audited v2 head):

    The v2 head declared its input as "1024x0e + 1024x1o + 1024x2e + 1024x3o"
    and fed it node_feats[:, 1024:]. Two things were wrong with that:

    1. An equivariant o3.Linear to 64x0e has weight paths only from 0e inputs,
       so only the first 1,024 dims (the block labelled 0e) ever carried
       weights — 65,600 = 1024*64 + 64 params. The higher-order features were
       structurally ignored.
    2. MACE concatenates node_feats as [layer0 full irreps (16,384),
       layer1 scalars (1,024)]. Slicing off the first 1,024 dims removes
       layer0's true scalars, so the block labelled 0e actually contained
       the first 1,024 components of layer0's 1o vectors. The head therefore
       read rotation-covariant components through scalar weights: delta was
       not rotation-invariant.

    This version feeds the FULL node_feats vector with correctly declared
    irreps. The o3.Linear then draws on the true scalars of both layers
    (2,048 dims -> 131,136 params) and delta is rotation-invariant by
    construction.

Output: ~/delta_head/delta_head_fixed_fw{FW}.pt  (head state_dict only)

Usage:
    python train_delta_head_fixed.py [--epochs 200] [--batch-size 64] [--lr 1e-3]
"""
import argparse
import os
import random
...
# True irreps of the full node_feats concat of mace_t1x_p10 (num_interactions=2):
#   layer 0: 1024x0e + 1024x1o + 1024x2e + 1024x3o   (16,384 dims)
#   layer 1: 1024x0e                                  ( 1,024 dims, scalars only)
HIDDEN_IRREPS = o3.Irreps("1024x0e + 1024x1o + 1024x2e + 1024x3o + 1024x0e")
MLP_IRREPS    = o3.Irreps("64x0e")
NODE_FEATS_OFFSET = 0   # full vector — the irreps declaration does the selecting
...
def forward_delta(batch, model, delta_head, training=True):
    ...
    with torch.enable_grad():
        ...
        node_feats = out['node_feats'][:, NODE_FEATS_OFFSET:]      # [N_atoms, 17408]
...
def main(args):
    ...
    # Delta head (fixed irreps declaration)
    ...
    print(f'Delta head (fixed): {n_params} parameters')  # expect 131,136
    ...
    wandb.init(
        ...
        config={
            ...
            'node_feats_offset': NODE_FEATS_OFFSET,
            'resume':        args.resume,
        },
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_f_f < best_force_loss:
            ...
            torch.save(delta_head.state_dict(), f'{OUT_DIR}/delta_head_fixed_fw{args.force_weight:.2f}.pt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--force-weight',  type=float, default=2.0)
