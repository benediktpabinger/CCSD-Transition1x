"""FIXED-HEAD VARIANT — derived from eval_benchmark_sp_fw2.py by constant substitution only.
Evaluates delta_head_fixed_fw2.00.pt (corrected irreps declaration, full node_feats).
See train_delta_head_fixed.py for the audit that motivated the fix.
"""
"""
Evaluate energies + forces on the 30 benchmark reactions, last 10 NEB images,
using the current fw=2.0 / 64x0e delta head (the one actually used in the
MACE+delta NEB benchmark) rather than the stale v1 16x0e checkpoint.
...
Four methods compared on every geometry:
  ...
  4. MACE + delta (fw=2.0)    — corrected energy + forces (autograd through head)

Metrics vs wB97M-V ground truth:
  - Relative energy MAE (meV), referenced to first image per reaction
  - Force MAE (meV/Ang), absolute

Output: ~/delta_head/eval_benchmark_sp_fw2.json

Usage:
    python eval_benchmark_sp_fw2.py
"""
import json
import os
import re
...
HEAD_PATH  = f'{HOME}/delta_head/delta_head_fixed_fw2.00.pt'
NEB_DIR    = f'{HOME}/orca_neb_results'
ENGRAD_DIR = f'{HOME}/mr_benchmark/orca_engrad'
OUT_PATH   = f'{HOME}/delta_head/eval_benchmark_sp_fixed_full.json'

N_IMAGES = 10
EH_BOHR_TO_EV_ANG = 51.42208619

HIDDEN_IRREPS     = o3.Irreps("1024x0e + 1024x1o + 1024x2e + 1024x3o + 1024x0e")  # full concat, true irreps
MLP_IRREPS        = o3.Irreps("64x0e")
NODE_FEATS_OFFSET = 0      # full node_feats vector (fixed head)
...
def main():
    ...
    delta_head.eval()
    print(f'Loaded MACE + delta head (fw=2.0, 64x0e) from {HEAD_PATH}\n')
