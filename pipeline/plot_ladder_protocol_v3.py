"""Flow diagram of the residual-force protocol (Section 4.1.5), v3: with a re-optimisation icon.
Level of theory and spin formalism large, everything else small.
Output: Pictures/fig_ladder_protocol_v3.png
...
# ---- icon: atoms shifting (hollow = before, filled = after), left of the arrow
from matplotlib.patches import Circle
ICON = "#444444"
ix, iy = XS[0] + W / 2 + 1.75, (Y1 - 0.75 + Y2 + H) / 2 + 0.30
before = [(-0.85, 0.42), (0.00, -0.45), (0.85, 0.38)]
shift  = [(-0.20, -0.62), (0.00, 0.66), (0.30, -0.58)]
for (bx, by), (dx, dy) in zip(before, shift):
    ax.add_patch(Circle((ix + bx, iy + by), 0.19, fc="white", ec=ICON, lw=1.1, ls=(0, (2, 2))))
    L = (dx**2 + dy**2) ** 0.5; ux, uy = dx / L, dy / L; r = 0.22
    ...
    ax.add_patch(Circle((ix + bx + dx, iy + by + dy), 0.19, fc=ICON, ec=ICON, lw=1.0))
# bonds between the shifted atoms
...
fig.savefig("Pictures/fig_ladder_protocol_v3.png", dpi=220, bbox_inches="tight")
print("written Pictures/fig_ladder_protocol_v3.png")
