#!/bin/bash
#SBATCH --job-name=mr_ccsdt_uma_s
...
#SBATCH --mem=120GB
#SBATCH --array=0-29
#SBATCH --output=/home/energy/s242862/logs/mr_ccsdt_uma_s_%A_%a.log

# CCSD(T)/def2-TZVP on R, TS, P at UMA-S NEB geometries (all 30 benchmark reactions).
# Tests whether UMA-S finds geometries that give the right barrier under a
# higher level of theory, independent of its own (foundation-model) energetics.
...
OUT=/home/energy/s242862/mr_benchmark/results/${RXN}_ccsdt_uma_s.json
if [ -f "$OUT" ]; then
...
python3 /home/energy/s242862/pipeline/mr_benchmark_ccsdt.py ${RXN} --n-threads 12 \
    --geom-dir /home/energy/s242862/uma_neb_results/${RXN} \
    --tag uma_s
