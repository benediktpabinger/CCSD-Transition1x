"""Boxplots of the disagreement among the three OMol25 models, split by whether
the reference transition state is a valid single-reference description.

Individual points are drawn next to each box: with 19 and 26 reactions the
distribution is carried by a handful of cases, and a box alone would hide that.
Log axes throughout, since every quantity spans three to four orders of
magnitude.
"""
import itertools
import json
    ...
    for a, b in itertools.combinations(xs, 2):
        ...
        if pairs:
            rr.append(max(
                ...
                for i, j in pairs))
    data[cls]['e'].append(max((max(ev) - min(ev)) * HA_MEV, 1e-3))
    data[cls]['g'].append(max(max(gg), 1e-4) if gg else np.nan)
    data[cls]['rc'].append(max(max(rr), 1e-4) if rr else np.nan)
    data[cls]['lab'].append(rx)

GROUPS = ['single-reference', 'multireference']
COL = {'single-reference': '#4878a8', 'multireference': '#c0553b'}
PANELS = [('e', 'Barrier spread', 'largest pairwise difference in\n'
                                  'DFT energy at the predicted TS  [meV]'),
          ('g', 'Transition-state geometry',
           'largest pairwise Kabsch RMSD  [Å]'),
...
fig.subplots_adjust(top=0.74, bottom=0.13, left=0.07, right=0.98, wspace=0.32)
...
for ax, (key, title, ylab) in zip(axes, PANELS):
    ...
    for i, v in enumerate(vals, start=1):
        ax.annotate(f'median {np.median(v):.4g}',
                    xy=(i, np.median(v)), xytext=(0, -16),
                    textcoords='offset points', ha='center', fontsize=8.5,
                    color='#333')

fig.suptitle('Disagreement among the three OMol25 models  '
             '(UMA-S, UMA-M, eSEN)', fontsize=15, y=0.965, x=0.07, ha='left')
fig.text(0.07, 0.895,
...
fig.savefig('/home/energy/s242862/model_spread.png', dpi=200,
            facecolor='white')
print('written: model_spread.png')
for g in GROUPS:
    print(f"{g:<18} n={len(data[g]['e'])}  "
          f"dE median {np.median(data[g]['e']):.3g} meV  max {max(data[g]['e']):.4g}")
