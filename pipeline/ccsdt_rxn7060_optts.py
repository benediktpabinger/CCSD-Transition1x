"""
CCSD(T)/def2-TZVP single points for rxn7060 at the CASSCF OptTS geometry.

Geometries:
  TS  = ~/nevpt2_optts_results/rxn7060_avas/ts_casscf_opt.xyz  (CASSCF-optimised)
  R/P = ~/orca_neb_results/rxn7060/reactant.xyz / product.xyz  (ORCA NEB endpoints)

NEVPT2(opt) = 3919 meV. R>TS>P MR pattern — CCSD(T) tests whether NEVPT2 is
biased upward by R being over-stabilised (R has 6 frac orbs vs TS=4, P=2).

Run on cluster: python3 pipeline/ccsdt_rxn7060_optts.py
"""
...
print(f'  (negative Delta expected: R-max AS over-stabilises R → NEVPT2 too high)')
print(f'{"="*60}')
