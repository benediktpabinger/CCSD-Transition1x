...
TZ version: uses 16 parallel processes and reduced maxcore for cc-pVTZ calculations.
"""
import json
import os
import shutil
...
def run_ccsd_calculation(atoms, label, basis='cc-pVTZ', directory='.'):
    """Run CCSD energy calculation using ORCA with 16 parallel processes"""
...
def main(args):
    ...
    output_dir = f"ccsd_tz_{args.reaction.replace('+', '_').replace('/', '_')}"
    os.makedirs(output_dir, exist_ok=True)

    results = []

    print(f"\nStarting CCSD calculations with {args.basis} basis set (16 procs)...")
    print("=" * 60)
...
if __name__ == "__main__":
    parser = ArgumentParser(description='CCSD/TZ calculations for a config range (SLURM array jobs)')
