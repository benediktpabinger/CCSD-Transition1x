"""
True CCSD/cc-pVTZ CI-NEB using PySCF analytic gradients.

Variant of ccsd_neb_pyscf.py with two changes vs the DZ version:
  - cc-pVTZ basis (larger, more accurate)
  - CI-NEB starts immediately (climb=True from step 0, no plain NEB phase)
    Safe because we warm-start from converged wB97x images — the band is
    already well-shaped and the climbing image won't overshoot.
...
Pipeline:
  ...
  3. Run CI-NEB directly with CCSD using NEBOptimizer (same as Transition1x)
  4. Save neb.db + fmaxs.json (identical structure to original wB97x pipeline)

Usage:
    python ccsd_cineb_tz_pyscf.py --h5file ~/data/Transition1x.h5 --reaction rxn0103 --output ~/ccsd_tz/rxn0103
"""
import argparse
import json
import os
