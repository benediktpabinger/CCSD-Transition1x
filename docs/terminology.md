...
- **Transition1x level** = \mbox{$\omega$B97X-D3/6-31G(d)}, functional and basis only. The level Transition1x was generated at, restricted. Defined at first use in Chapter 4, Section "Residual forces at the training geometries".
...
- **Levels always with \omega**: \mbox{$\omega$B97X-D3/6-31G(d)} and \mbox{$\omega$B97M-V/def2-TZVP(D)} in prose; in math superscripts $E^{\omega\text{B97X}}$, $E^{\omega\text{B97M}}$ (Chapter 3 error split). Plain "wB97" only in ORCA input lines.
