% Background, Endfassung im Aufbau. Ein Begriff pro Absatz, drei bis fuenf
% Saetze, keine Verweise auf Ergebnisse der Arbeit. Referenzen:
% docs/background_refs.bib. Verlauf: docs/chapter_background_v2.tex,
% Thesis-Kopie: docs/chapter_background.tex.
...
Every electronic structure method approximates
Eq.~\eqref{eq:electronic-schrodinger}; an exact solution exists for one
electron only. A method restricts the form of the wavefunction and returns
the lowest energy within that form, with its gradient. If the true ground
state lies outside the form, the method returns a higher state without
indication.
...
The converged result is a stationary point of the energy with respect to the
orbitals. Two properties of the iteration:

\begin{itemize}
  \item it converges to the stationary point the starting guess leads to;
  \item a stationary point is not necessarily the lowest one.
\end{itemize}

Equation~\eqref{eq:ks-energy} and the SCF iteration leave three things
unspecified (Figure~\ref{fig:choices}). The term $E_\text{XC}$ needs a
formula; this is the choice of functional
(Section~\ref{sec:background-functionals}). The orbitals $\phi_i$ need a
mathematical form in which they can be stored and optimised; this is the
choice of basis set (Section~\ref{sec:background-basis}). And each orbital
carries a spin; whether spin-up and spin-down electrons must share the same
orbital or may have different ones is the choice of spin treatment
(Section~\ref{sec:background-spin}). The first two are named in the level
of theory, written functional/basis.

\begin{figure}[h]
\centering
% Preamble needs: \usepackage{tikz} \usetikzlibrary{calc, fit, positioning, decorations.pathreplacing, arrows.meta, backgrounds}
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[
...
\node[box, fill=orange!8, anchor=west] (spin)  at ($(eq.east |- exc.center)+(1.2,-4.2)$)
  {\textbf{Spin treatment}\\ $\alpha$, $\beta$ forced to share orbitals\\ (RKS) or not (UKS)};
...
from the orbitals of both spins, and the spin treatment decides whether
...
density. The suffix -D3 or -V in a functional name says which.

Three levels of theory appear in this thesis:

\begin{itemize}
  \item \textbf{$\omega$B97X-D3/6-31G(d)}~\cite{lin2013}: range-separated
    hybrid GGA with D3 dispersion, small basis. The level of Transition1x,
    chosen for the cost of ten thousand NEB runs.
...
represented at all. It is the second approximation of a Kohn--Sham
calculation, and it is independent of the first.
...
functions. A basis set is described by three properties:

\begin{itemize}
  \item \textbf{Functions per valence orbital.} One function fixes the
    orbital to its shape in the free atom. Two functions of different width
    (double zeta) let it expand or contract in the molecule; three (triple
    zeta) more so. The core orbitals usually keep one function.
...
functional/basis, for example \mbox{$\omega$B97X-D3/6-31G(d)}. The two
...
(Table~\ref{tab:rks-uks}). Programs use RKS by default for multiplicity
one~\cite{orca_manual, qchem_manual}.

\begin{table}[h]
\centering
\small
\caption{The two spin treatments and what they can describe.}
\label{tab:rks-uks}
\begin{tabular}{@{}lcc@{}}
\toprule
 & RKS & UKS \\
...
% TODO (mit Literatur): Absatz "Broken symmetry and the multireference
% case" -- open-shell singlet = MR-Fall aus sec:sr-vs-mr; BS als
% Ein-Determinanten-Naeherung, MR-Methoden als exakte Behandlung.
...
solution exists, $\langle S^2 \rangle > 0$.

\paragraph{Wavefunction, not geometry.} The stability analysis is a
statement about the electronic solution at one fixed geometry. Whether
that geometry is also a stationary point of the broken-symmetry surface is
a separate question, answered by the gradient of the broken-symmetry
solution at that geometry.
...
used a coupled cluster method and a multireference method in addition to
DFT; neither enters the final results. They are described here as far as
...
\paragraph{CASSCF and NEVPT2.} In the multireference case no single
determinant is a good starting point, and CASSCF starts from several.
Only a few orbitals change their occupation between the configurations
that mix, for a breaking bond the bonding and the antibonding orbital of
that bond; these are chosen as the active orbitals. All lower orbitals
stay doubly occupied, all higher ones empty, and the active electrons are
distributed over the active orbitals in every possible way. Each
distribution is one determinant, and CASSCF optimises their weights and
...
\end{equation}
%
CASSCF mixes only the determinants of the active space, the few with
large weight. The many determinants outside it, the small corrections of
Section~\ref{sec:sr-vs-mr}, are missing, and CASSCF energies alone are not
accurate. NEVPT2~\cite{angeli2001} estimates their contribution
afterwards by perturbation theory on top of the CASSCF wavefunction. The
result depends on which orbitals are active. For a single geometry that
is a matter of choice; for a reaction path it is a condition: the same
orbitals must stay active at every geometry, otherwise the surface changes
its definition along the path. Choosing the active space at one geometry
is automated~\cite{sayfutyarova2017, stein2019autocas}; keeping it
consistent along a path is not~\cite{bensberg2023, seal2025wasp}.
...
\begin{itemize}
  ...
  \item $N_\text{FOD}$~\cite{grimme2015}: computed from one DFT
    calculation at a finite electronic temperature of 5000~K. At zero
    temperature every orbital is either occupied by two electrons or
    empty. At finite temperature the electrons are spread over the
    orbitals near the gap between occupied and empty: an orbital far
    below the gap stays at occupation 2, one far above at 0, but orbitals
    close to the gap take fractional occupations $n_i$ between 0 and 2.
    In a single-reference molecule the gap is large and nothing changes.
    In a multireference case the frontier orbitals are close in energy,
    exactly the orbitals that would be active in CASSCF, and they take
    fractional occupations. The sum of the deviations from the integer
    values $n_i^0$,
    %
    \begin{equation}
    ...
    \end{equation}
    %
    is therefore near zero for a single-reference molecule and grows with
    the number of electrons in near-degenerate orbitals.
\end{itemize}
...
A saddle point cannot be found by descending the surface. If reactant and
product are known, a path between them can be optimised instead: the
...
\end{equation}
%
This image moves uphill along the path and downhill perpendicular to it,
and converges to the saddle point.
...
\begin{itemize}
  ...
  \item the model has no wavefunction, no density and no spin treatment.
...
\paragraph{Message passing.} The list is filled in layers. In every layer
an atom receives from each neighbour within the cutoff the neighbour's
features together with the distance and direction to it, a message, and
updates its own features from them. After one layer an atom knows its
direct neighbours, after two layers also their neighbours.
PaiNN~\cite{schutt2021painn} made the messages equivariant, so that
directional information is carried from atom to atom.
...
channels of each kind; with 1024 per $\ell$ up to 3, the list of one atom
holds $1024 \times (1 + 3 + 5 + 7) = 16\,384$ numbers per layer.
...
run at the \mbox{$\omega$B97X-D3/6-31G(d)} level in ORCA. The level was
chosen for the cost of twelve thousand NEB runs. The band converged for
...
enumeration and reaction paths; none is relaxed at the OMol25 level.
...
community & ANI-2x, ANI-1xBB, Transition1x, OrbNet Denali, SPICE2,
  Solvated Protein Fragments, about 30\,\% of GEOM, recomputed at the
  OMol25 level & 1.4\,B \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
MACE & Transition1x & $\omega$B97X-D3/6-31G(d) & base model, Chapter~\ref{ch:delta} \\
