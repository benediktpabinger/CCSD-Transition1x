...
this work. Smaller molecules in this part of the test set would have avoided
this. NEVPT2, the standard protocol for energies on unrestricted
geometries, builds on a CASSCF wavefunction and therefore needs the
same active space, so it was not carried out either.
...
too. Within the high-MR tier, restricted to the range of
multireference character both groups share, the broken-symmetry
structures remain worse by a factor of about 1.6, and that part is
what the training data explain.

Figure~\ref{fig:pes} gives the intuition: a schematic of the same
geometries on the surface they were relaxed on and on the surface the
