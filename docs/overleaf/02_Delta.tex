...
\mbox{$\omega$B97X-D3/6-31G(d)}, and are not re-optimised at the new
...
Throughout this thesis, MACE is the MACE model trained on
Transition1x at \mbox{$\omega$B97X-D3/6-31G(d)}, the base model of the
correction, and MACE+$\Delta$ is the same model with the correction
head attached, approximating \mbox{$\omega$B97M-V/def2-TZVP}. Base model and correction
head combine into one corrected potential; the superscript denotes the
level of theory a prediction approximates:
%
\begin{equation}
...
the dataset's \mbox{$\omega$B97X-D3/6-31G(d)} energies and forces as targets. A full
...
\paragraph{An earlier version of the head.} It declared its input
...
One limitation applies: for reactions with strong multireference
character the reference itself is single-reference DFT, restricted.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ll}
    ...
    Starting band & final $\omega$B97X-D3/6-31G(d) NEB images from Transition1x \\
...
The correction is expected to behave differently at high and low
...
$\omega$B97X-D3/6-31G(d) --- does it reproduce $\omega$B97X-D3/6-31G(d) energies and forces well?
...
of the NEB run itself; the $\omega$B97X-D3/6-31G(d) energies and
forces are not available, so we compute them as single points on all
300 geometries with ORCA. MACE is also compared against the
$\omega$B97M-V/def2-TZVP reference as a control.
...
\paragraph{Evaluating performance on single-point predictions on fixed geometries: how well does MACE reproduce its target level $\omega$B97X-D3/6-31G(d)?}
...
geometries, MACE deviates from the $\omega$B97X-D3/6-31G(d)
...
140\,meV/\AA{} ($\omega$B97X-D3 rows of Table~\ref{tab:delta-sp-results}). The
...
the energy profile. The forces are the clear win: they improve on all 30 reactions, in every
...
the MACE error stays small, but the head predicts a larger gap than
there is, so the corrected barrier goes past the reference. At high
MR the MACE error is large as well, and both errors push the barrier
below the reference.\footnote{The average
...
$\omega$B97X-D3/6-31G(d) itself lies within 42\,meV of the reference, while MACE misses its
own level by 514\,meV.\label{fn:all30}}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{Pictures/bias_decomposition_v4.png}
  \caption{Fixed geometries, barrier bias decomposed via
  Eq.~\eqref{eq:error-split}, by MR tier. Dashed lines mark the two
  levels of theory; both models start at the $\omega$B97X-D3/6-31G(d) level.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llrrrr}
    ...
    \midrule
    low  & $\omega$B97X-D3      &  80.7 & 107.2 & 136.0 & $+136$ \\
         & MACE          &  86.0 & 104.2 & 169.8 & $+170$ \\
         & MACE+$\Delta$ &  29.6 &  31.6 &  47.0 & $+21$ \\
    \midrule
    mid  & $\omega$B97X-D3      & 120.1 & 151.1 & 193.4 & $+185$ \\
         & MACE          & 148.2 & 151.9 & 254.0 & $+151$ \\
         & MACE+$\Delta$ &  54.3 &  53.0 & 147.1 & $-134$ \\
    \midrule
    high & $\omega$B97X-D3      &  83.9 & 162.4 & 124.4 & $+124$ \\
         & MACE          &  88.7 & 160.4 &  81.2 & $+21$ \\
         & MACE+$\Delta$ &  82.8 &  59.3 & 186.3 & $-184$ \\
    \midrule
    all\textsuperscript{\ref{fn:all30}}  & $\omega$B97X-D3      &  94.9 & 140.2 & 151.3 & $+148$ \\
...
\begin{figure}[htbp]
  ...
  of the same reactions as grouped bars (left to right: $\omega$B97X-D3/6-31G(d) DFT, MACE,
...
\begin{figure}[htbp]
  ...
  $\omega$B97X-D3/6-31G(d) single points, i.e.\ the level gap itself. MACE sits on the
...
\paragraph{Why bare MACE is closer at high MR.} On the high-MR
reactions the uncorrected model is closer to the reference than the
corrected one. It is right for the wrong reason. The cheap level lies
$+124$\,meV above the reference in this tier
(Table~\ref{tab:delta-sp-results}). MACE is trained to reproduce the
cheap level, so it should lie there too; it lies at $+21$\,meV,
because it misses its own level by about $-100$\,meV. That error
happens to cancel most of the gap, and bare MACE lands near the
reference by accident. The head then subtracts the gap, as it was
trained to, from a MACE barrier that is already $100$\,meV too low,
and the corrected barrier ends at $-184$\,meV. Two errors that cancel
in the uncorrected model add up in the corrected one. The NEB-driven
searches show the same: MACE at $-3$\,meV, MACE+$\Delta$ at
$-235$\,meV (Table~\ref{tab:delta-neb-results}).

\paragraph{Where the correction stops.}
Both evaluations draw the same line: the correction works where
multireference character is low and overshoots where it is high. Why
the head corrects too far there is not resolved in this chapter. Both
labels are restricted at every geometry, so the difference is well
defined and learnable in principle; the cause is not in the labels.
The references are restricted as well, by construction, and whether
the restricted solution is the ground state at those transition states
was not tested here. That is the question of Chapter~\ref{ch:mr}.
...
% 300 structures. The wB97X-D3 column is a level of theory rather than a model, and gives
...
% of \mbox{wB97X-D3} against \mbox{wB97M-V} (0.3216) to three decimals, and the force MAEs
...
% \mbox{wB97X-D3} & $+148.2$ & 151.3 & $+231.8$ & 231.8 \\
% \hline
% \end{tabular}
% \end{table}

% The ratio of bias to MAE reads directly as the systematic fraction of the error.
% Against CCSD(T), \mbox{wB97X-D3} sits at $231.8/231.8 = 1.00$: every one of the 30
...
% below \mbox{wB97X-D3}'s 124.4\,meV on the same reactions. Its energy MAE on that tier
...
% difference between two single-reference functionals. Where the \mbox{wB97X-D3} labels
...
% \begin{tabular}{lcc|cc}
% \hline
%  & \multicolumn{2}{c|}{vs \mbox{wB97M-V}} & \multicolumn{2}{c}{vs \mbox{wB97X-D3}} \\
...
% \mbox{wB97X-D3} & 98.8 & 130.9 & --- & --- \\
% \hline
% \end{tabular}
% \end{table}

% Read across the rows rather than down the columns. UMA-S disagrees with
% \mbox{wB97X-D3} by 99.4\,meV and 131.1\,meV/\AA; the two functionals disagree with each
% other by 98.8\,meV and 130.9\,meV/\AA. The same holds for UMA-M and eSEN. Their
% disagreement with \mbox{wB97X-D3} is the functional gap, which places them on the
...
% defined in Equation~\ref{eq:delta-def} and nothing else. Where the \mbox{wB97X-D3}
...
% Transition1x \mbox{wB97X-D3} path geometries and evaluated on ORCA \mbox{wB97M-V} path
