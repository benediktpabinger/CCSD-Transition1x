...
MACE model trained on Transition1x at \mbox{$\omega$B97X-D3/6-31G(d)}.
...
+ \Delta^{\,\omega\text{B97M-V/def2-TZVP}\,-\,\omega\text{B97X-D3/6-31G(d)}}(\mathbf{R})
...
- \nabla_{\mathbf{R}}\Delta^{\,\omega\text{B97M-V/def2-TZVP}\,-\,\omega\text{B97X-D3/6-31G(d)}}(\mathbf{R})
...
of the NEB run itself. To also have $\omega$B97X-D3/6-31G(d) energies
...
against \mbox{$\omega$B97X-D3/6-31G(d)}, MACE+$\Delta$ against
...
\begin{figure}[htbp]
  ...
  target level, $E_{\text{MACE}} - E^{\omega\text{B97X}}$ (teal).
  MACE+$\Delta$ carries two errors: the same baseline error
  $E_{\text{MACE}} - E^{\omega\text{B97X}}$, and the head's error in predicting
  the level-of-theory gap,
  $\Delta - (E^{\omega\text{B97M}} - E^{\omega\text{B97X}})$, the dark part of the
  correction arrow; the light part removes exactly the gap,
  $E^{\omega\text{B97M}} - E^{\omega\text{B97X}}$. At low MR the dark part is
...
\paragraph{Origin of the overshoot.} At mid MR the base model is
accurate and the head predicts a larger level gap than there is. The
error is the head's alone. At high MR the base model already misses
its own level by about $-100$\,meV, and the head subtracts the full
gap on top of it. The two errors add to $-184$\,meV
(Figure~\ref{fig:bias-decomposition}). This is also why bare MACE
looks right at high MR: the cheap level lies $+124$\,meV above the
reference, and MACE's own error cancels most of that gap. MACE lands
near the reference by accident. The model-driven searches show the
same, MACE at $-3$\,meV and MACE+$\Delta$ at $-235$\,meV
...
is the lowest, so the reference is sound and the overshoot is the
head's error. At seven of the ten high-MR ones a lower
