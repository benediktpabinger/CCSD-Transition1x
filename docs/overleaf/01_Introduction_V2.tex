...
\mbox{$\omega$B97X-D3/6-31G(d)} (Section~\ref{sec:mace-training}), and
...
Transition1x, a dataset labelled restricted at a cheap level of
theory, and relabelled it unrestricted at an expensive one, again
without re-optimising a single path. Two questions lie underneath this
...
RQ1 was the task this thesis was set. RQ2 arose during that work: the
OMol25 models, used along the way as a point of comparison, did not
find the restricted reference transition states on some reactions but
different structures. The reason is that their labels are
unrestricted, so the models search for transition states with broken
spin symmetry, while the references were optimised with restricted
spin symmetry. Whether the models can find those broken-symmetry
...
this chapter shows (see the Preface).
...
and RQ2 becomes a real question. OMol25
is exactly this case: every label recomputed unrestricted at
\mbox{$\omega$B97M-V/def2-TZVPD}, on the inherited Transition1x
geometries and on many structures beyond them~\cite{omol25}.
Chapter~\ref{ch:mr} asks RQ2 there.
...
state of the ground-state surface and should not serve as a
...
stationary points of the ground-state surface: the force there is
...
have different ones is the choice of spin treatment
...
\paragraph{Active orbitals.} Only a few orbitals change their occupation
between the configurations that mix, for a breaking bond the bonding and
the antibonding orbital of that bond. These are the active orbitals.
CASSCF keeps all lower orbitals doubly occupied and all higher ones empty,
and distributes the active electrons over the active orbitals in every
possible way.

\paragraph{CASSCF and NEVPT2.} In the multireference case no single
determinant is a good starting point. CASSCF starts from all
determinants of the active space at once,
%
\begin{equation}
  \Psi_\text{CAS} = \sum_{I \in \text{active}} c_I \,\Phi_I ,
\end{equation}
%
and optimises the weights $c_I$ and the orbitals together. This captures
the mixing of the few large determinants. NEVPT2~\cite{angeli2001} adds
the remaining correlation afterwards by perturbation theory. The result
depends on which orbitals are active. For a single geometry that is a
