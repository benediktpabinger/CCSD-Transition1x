...
% VORSCHLAG neue Gliederung (Fassung 2), 2026-09-20. Zeilen mit '% >>>' markieren alles, was nicht
% woertlich aus 01_Introduction_V2.tex stammt oder verschoben wurde. Vor dem Uebernehmen loeschen.
...
% >>> SPLIT into two sentences; "It demonstrated" -> "Transition1x demonstrated" so the subject stays clear
generated by running nudged elastic band (NEB) searches. Earlier
datasets held equilibrium structures and their surroundings.
% >>> ADDED "in the transition-state region" (approved, option A); otherwise the original sentence
...
% >>> CHANGED one word: "cheaper" -> "easier" (approved)
...
% >>> MOVED: RQ1 box, now directly after relabelling is defined
\noindent
...
% >>> SAME PLACE as before. NEW: the context in front (largest dataset, frontier models, "Transition1x is one
% >>> of the recomputed datasets:"). From "OMol25 relabelled the entire Transition1x dataset at" on it is the
% >>> original sentence with both footnotes.
...
% >>> UNCHANGED, including "we pursue a different goal"
...
% >>> NEW bridge. First and last sentence are new; the middle is the old paragraph "RQ1 was the task this
% >>> thesis was set. RQ2 arose ..." plus the two sentences now also in the Preface.
The introduction so far describes what this thesis was set out to do.
...
and our references describe different surfaces. Whether these models
...
against restricted references, as the rest of this chapter shows; the
Preface gives the account. We therefore formulated a second research
question.
...
% >>> UNCHANGED except that the last sentence ("Two questions lie underneath this practice, and this thesis
% >>> asks both:") is gone. "OMol25 went one step further: ..." stays.
...
enough to lift the model to the higher level of theory. That is how the field raises fidelity: transfer
...
a median of 0.007\,\AA{} apart. A path relaxed at the cheap level
...
% >>> UNCHANGED, including "OMol25 is exactly this case: ..."
...
question. OMol25 is exactly this case: every Transition1x label
recomputed unrestricted at \mbox{$\omega$B97M-V/def2-TZVPD}, on the
inherited geometries~\cite{omol25}. Chapter~\ref{ch:mr} asks RQ2 there.

% >>> REMOVED here: the paragraph "RQ1 was the task this thesis was set. RQ2 arose ..." (now the bridge above).
% >>> UNCHANGED from here to the end of the chapter.
...
% >>> ADDED the closing clause (approved, option A)
...
% >>> REMOVED "that never tested the solution" (approved)
...
% >>> ADDED "to teach a model to find broken-symmetry transition states" (approved)
...
% >>> SIMPLIFIED the stability-analysis sentence (approved, option A)
...
% >>> NEW sentence on multireference character (2026-09-21)
Part of this difference is multireference character itself, which is
highest at the broken-symmetry reactions, but between structures of
equal multireference character a difference remains.

% >>> NEW paragraph; "The chapter traces this to the training data." replaced by
% >>> "The chapter examines two candidate causes for it. The first is the training data."
...
% >>> NEW: the second candidate, same wording as the abstract
