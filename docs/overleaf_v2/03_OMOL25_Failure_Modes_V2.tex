...
dataset of more than 100 million DFT calculations. It relabelled
Transition1x in full (training, validation and test split alike, all
in its training split), with every label recomputed unrestricted at a
higher level. We train nothing ourselves: we take three published
models trained on OMol25, UMA-S, UMA-M and eSEN, and test them. This
removes the first of the two causes: every label is relabelled and the
models are large ones trained on all of it, so neither a subset nor a
small head can be blamed. What remains is the second, training paths
that were never relaxed on the surface of their labels, which is RQ2
...
\begin{itemize}
  \item the DFT forces. They show whether the structure is a stationary
    ...
    else. The model predicts forces at this geometry as well, and the
    difference between the model forces and the DFT forces is the
    model's force error.
  \item the DFT energy. It gives the barrier of the model's own
    geometries on that surface. The model predicts this barrier as well,
    and the difference between the model barrier and the DFT barrier is
    the model's energy error.
...
from stationarity on that surface.
...
points are specific to a surface: the stored transition states need not
be stationary points of the label surface, whose transition state may
lie elsewhere in configuration space. A model learns the surface its
labels describe, so it learns the label surface from points displaced
from its transition state rather than from the transition state itself.
...
and rxn8885, are reactions whose bands did not converge. In the 37
reactions where all three searches converged, the median spread is 0.33
and 4.59~meV, a factor of 14, with 13 of the 14 broken-symmetry
reactions above 1~meV against four of the 23 closed-shell ones, and two
reactions of each group above chemical accuracy.
...
level, so a change of level alone does not keep a model from learning a
surface from paths that were not relaxed on it. At broken-symmetry
...
separates the groups (Table~\ref{tab:ladder}). The same holds at equal
multireference character: in the $N_\mathrm{FOD}$ range that both groups
share, the stored transition states of the broken-symmetry reactions
carry 1.70~eV\,\AA$^{-1}$, against 0.58 for the closed-shell ones. The
