...
Transition1x to predict the lower level (Section~\ref{sec:mace-training}) --- the model whose level of theory we upgrade. It
...
\end{equation}
These differences are the training targets.\footnote{On the 225
validation reactions, $\Delta E$ is about $-3.0$~eV per molecule, and
within one reaction its standard deviation is 0.09~eV on average and
0.25~eV at most.}

We sampled 5{,}000 reactions at random from the Transition1x training split
(fixed seed) and selected up to 17 of the ${\sim}950$ path geometries per reaction.
...
from each, end points included (Figure~\ref{fig:stratified-sampling}).
Neighbouring stretches share an end point, so the 20 picks give at most
17 distinct geometries, and fewer where a stretch is short. The two stretches at the
...
uniform selection would give no such guarantee.
...
target. This amounts to about 80{,}000 DFT calculations, 3--4 days on the
...
split (10{,}600 geometries, 2{,}240 of them with force
...
The last two rows of Table~\ref{tab:delta-training}, scheduler signal
and checkpoint criterion, use different validation quantities on
purpose. The
...
Having constructed and trained MACE+$\Delta$, we now evaluate it against the target level. We run $\omega$B97M-V/def2-TZVP reference
...
test split (Table~\ref{tab:reference-neb}). This level of theory is the natural reference: it is exactly
...
We use the diagnostic $N_{\mathrm{FOD}}$ to characterise it
(Section~\ref{sec:background-beyond}). We compute it at the
reference transition states (PBE/def2-SVP, Fermi smearing,
$T_{\mathrm{el}} = 5000$\,K), where we expect
...
and therefore serves as a base that predicts the lower level
...
target level. For the forces and the energies it does in every tier. For
the barriers it does at low multireference character, and it overshoots
at mid and high (Table~\ref{tab:delta-sp-results}).
...
by about two thirds at low MR (86 $\rightarrow$ 30\,meV) and mid MR
(148 $\rightarrow$ 54\,meV), only marginally at high
...
\begin{figure}[H]
  ...
  error grows with multireference character, and at high MR, MACE
...
and a barrier. All 30 runs deliver a transition state, with MACE and
with MACE+$\Delta$; 24 of the 30 reach the force criterion within the
step budget, for either model.

We look at the geometries first. Every number below is the Kabsch RMSD
between two transition-state geometries of the same reaction, given as
the median over the 30 reactions; which two is stated in each case.
Before asking whether the correction improves the
geometries, we ask how much they could improve at all. Three
distances answer this:

\begin{itemize}
  \item lower-level DFT transition state, as stored in Transition1x, to
    higher-level DFT transition state: 0.007\,\AA{}. This is how far
    the level upgrade moves the transition state, and the most a
    correction could gain.
  \item MACE transition state to lower-level DFT transition state:
    0.015\,\AA{}. This is MACE's error against its own level.
  \item MACE transition state to higher-level reference:
    0.017\,\AA{}. This is MACE's error against the target.
\end{itemize}

MACE's error against its own level, 0.015\,\AA, is already twice the
most the correction could gain, 0.007\,\AA, so it cannot improve the
geometries by much; it can only make them worse. It does not: in Figure~\ref{fig:rmsd-parity} almost all
...
On the mid- and high-MR reactions the corrected barriers lie about
200\,meV below the reference, and on the high tier MACE without the
correction is closer to the reference than MACE+$\Delta$.
The decomposition behind Figure~\ref{fig:bias-decomposition} applies
here as well: the barriers at the transition states the models found
themselves show the same pattern as at the fixed geometries, with both
models 25 to 105~meV lower at mid and high MR. That the correction improves the low-MR barriers without damaging
...
\emph{Section~\ref{sec:delta-architecture} described the earlier head, whose
...
head, NEB found structures twice as far from the reference as MACE;
...
answered yes, with the reservation of the mid- and high-MR barriers
discussed next.

\paragraph{Dependence on multireference character.} The corrected model struggles where the
...
The mid tier is the typical reaction, and there the
...
\paragraph{Origin of the overshoot.} The correction head does what it
was trained to do in every tier: its error against the true level gap,
the difference between the $\omega$B97M-V/def2-TZVP and the
$\omega$B97X/6-31G(d) barrier, is $+25$, $-43$ and $+7$\,meV at low, mid
and high MR. The overshoot
...
This is also why MACE looks right at high MR: the lower level
...
\paragraph{Reliability of the reference.} Every error in this chapter
is measured against a restricted reference. At high multireference
character a restricted determinant can describe the molecule poorly,
and a lower broken-symmetry solution may exist. This chapter cannot
detect that, because every calculation in it is restricted; the
stability analysis of Chapter~\ref{ch:mr} can. At nine of the ten
mid-MR transition states the restricted solution is the lowest, so the
reference is sound. At seven of the ten high-MR transition states a
lower broken-symmetry solution exists. Two things follow. First, the
errors are errors of the models in both tiers, mostly the base model's:
the models were trained on restricted labels and are compared with a
restricted reference, so the restricted surface is all they are asked
to reproduce, and they still miss it. Second, and independently of
that, at those seven transition states reproducing the reference would
not have been enough either, because the restricted reference is itself
not the lowest DFT solution there.
