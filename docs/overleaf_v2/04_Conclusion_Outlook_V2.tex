% >>> DRAFT: restructured conclusion, NOT yet adopted. Do not upload to Overleaf
% >>> together with 04_Conclusion_Outlook.tex (duplicate labels).
% >>> Every change is marked with '% >>>'. All unmarked text is word for word
% >>> the text of 04_Conclusion_Outlook.tex (state of 2026-09-21).
...
% >>> CHANGE 2: "this chapter" -> "Chapter 2" (the sentence came from Chapter 2;
% >>> here "this chapter" would mean the conclusion)
...
% >>> CHANGE 4: paragraph MOVED up from the end of the chapter, so that the chapter
% >>> does not end on limitations. One word changed: "the test named above" ->
% >>> "the test named below" (the test now stands in the future-work paragraph).
...
% >>> CHANGE 5: NEW section heading. What the findings mean for the field (the first
% >>> two paragraphs) and what should follow (the rest) are collected here.
\section{Implications and outlook}

% >>> (unchanged; was after "Unrestricted labels need new paths")
...
% >>> CHANGE 5b: heading retitled, was "No benchmark until now has shown this error."
% >>> (it echoed the first sentence of CHANGE 7). Text unchanged, first paragraph
% >>> only; its second paragraph is CHANGE 7.
...
% >>> CHANGE 6: NEW lead-in sentence that ties the following paragraphs together
...
% >>> CHANGE 7: NEW heading; the text below it is unchanged (was the second
% >>> paragraph of "No benchmark until now has shown this error")
...
% >>> CHANGE 7b: NEW "and closes the gap indirectly: ... needs no reference saddle."
% >>> and "close it directly and" in the last sentence
...
% >>> CHANGE 8: heading retitled, was "Recommendation: A stability analysis before
% >>> trusting a saddle."; text unchanged
...
% >>> CHANGE 9: NEW final paragraph. It replaces the paragraph "Unrestricted labels
% >>> need new paths", whose recipe (one unrestricted CI-NEB per affected reaction,
% >>> stability analysis at the saddle) and whose fine-tuning test are folded in here.
...
% >>> CHANGE 9b: NEW closing sentence (the same paths also give the benchmark)
...
% >>> REMOVED by CHANGE 9 (kept here as a comment for comparison):
% \paragraph{Unrestricted labels need new paths.} If the training data are
% the cause, a
...
% stationary points. Whether this removes the errors has not been tested.
% Fine-tuning the models on such paths is that test, and it also
% separates the two causes: if the training data are the cause, the
% errors should disappear; if the shape of the surface is the cause,
% they should remain.
