> **2026-09-18, level correction.** Transition1x was computed at ωB97X/6-31G(d) *without* D3 (Schreiner et al. 2022, Chai & Head-Gordon functional). The single points behind this README's `wB97X-D3` rows were run with `! wB97X-D3 6-31G(d)` (ORCA's reparametrised ωB97X-D3), i.e. not at MACE's training level. They were rerun with `! wB97X 6-31G(d) TightSCF EnGrad` (cluster `~/mr_benchmark/orca_engrad_nod3`, SLURM 10852380; evaluation `~/pipeline/delta/eval_benchmark_sp_fixed_nod3.py`, job 10852531) → `eval_benchmark_sp_fixed_full_nod3.json`. The thesis (Chapter 3 fixed-geometry table, baseline paragraph, bias decomposition and the two dumbbell figures) uses the no-D3 file; the scripts are `pipeline/plot_delta_bias_decomposition.py`, `pipeline/plot_delta_barrier_dumbbell.py`, `pipeline/plot_delta_force_dumbbell.py`. MACE and MACE+Δ numbers are unchanged; the cheap-level rows change to (eMAE/fMAE/barrier MAE/bias) low 89.2/108.4/173.2/+173, mid 133.6/154.1/241.7/+242, high 113.0/160.9/212.6/+213, all 112.0/141.1/209.2/+209; MACE vs own level 42 meV / 34 meV/Å; level gap 112 meV / 141 meV/Å. The tables below are the old D3 numbers.
